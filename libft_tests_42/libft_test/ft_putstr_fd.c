...
typedef struct s_putstr_fd_param
{
	int fd;
	char *str;
...
TestSuite(ft_putstr_fd, .timeout=TIMEOUT);

ParameterizedTestParameters(ft_putstr_fd, simple)
{
	static t_putstr_fd_param data[] =
		{
			{
				.fd = 42,
				.str = "abcdef",
	...
	return cr_make_param_array(t_putstr_fd_param, data,
...
ParameterizedTest(t_putstr_fd_param *param, ft_putstr_fd, simple)
{
	...
	forbid_malloc();
	forbid_free();
	ft_putstr_fd(param->str, param->fd);
	allow_malloc();
	allow_free();
