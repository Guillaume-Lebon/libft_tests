...
ParameterizedTest(t_substr_param *c, ft_substr, simple)
{
	...
	set_real_function(&real_malloc, "malloc");
...
Test(ft_substr, edge_case0)
{
	...
	cr_assert(res != NULL, "Wrong return value got %p expected not %p.", res,
		NULL);
	cr_assert(c == '\0', "Wrong char got %d expected %d.", c, '\0');
}

Test(ft_substr, edge_case1)
{
	void *res;
	char c;
