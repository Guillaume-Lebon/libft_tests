...
typedef struct s_putchar_fd_param
{
	int fd;
	char c;
...
TestSuite(ft_putchar_fd, .timeout=TIMEOUT);

ParameterizedTestParameters(ft_putchar_fd, simple)
{
	static t_putchar_fd_param data[] = {
		{
			.fd = 42,
			.c = '@',
			.result =
	...
	return cr_make_param_array(t_putchar_fd_param, data,
...
ParameterizedTest(t_putchar_fd_param *param, ft_putchar_fd, simple)
{
	...
	mmk_when(write(mmk_any(int), mmk_any(const void *),
		mmk_any(size_t)), .then_call  = (void (*)(void))&mock_write);
	forbid_malloc();
	forbid_free();
	ft_putchar_fd(param->c, param->fd);
	allow_malloc();
	allow_free();
	...
	cr_assert(write_str_len == param->result.n,
		"Wrong write length got %d expected %d.", write_str_len, param->result.n);
